# PerfectMail: Remaining Performance Work (post P0/P1)

## Context

PerfectMail is a native macOS Gmail client (SwiftUI, Gmail REST API, GRDB/SQLCipher). Hot paths: `MailStore.swift` (UI state + queries), `SyncEngine.swift` (sync), `Database.swift` (schema), `ThreadDetailView.swift` (reading pane).
...
- Build/test: `make gen` (xcodegen), `make test` (hostless `PerfectMailTests`). Any new `Sources/` file used by unit tests **must** be listed under `PerfectMailTests` sources in `project.yml`, then `make gen`.
...
**Where:** `SyncEngine.swift` (and tests under `PerfectMailTests` if you extract a batch helper).
...
| Area | Primary files |
|------|----------------|
| Schema / migrations | `Sources/PerfectMail/Store/Database.swift`, `Tests/.../DatabaseMigrationTests.swift` |
| Sync / backfill | `Sources/PerfectMail/Gmail/SyncEngine.swift`, `GmailClient.swift` |
| List / badge / VIP | `Sources/PerfectMail/App/MailStore.swift` |
| Reading pane | `Sources/PerfectMail/UI/ThreadDetailView.swift`, `Support/WebViewPool.swift` |
| Derivation invariants | `Tests/.../ThreadDerivationTests.swift` |
| Test target list | `project.yml` → `PerfectMailTests.sources` |
