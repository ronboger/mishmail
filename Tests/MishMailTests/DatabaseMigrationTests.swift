...
final class DatabaseMigrationTests: XCTestCase {
    ...
    func testFreshDatabaseMigrates() throws {
        ...
        try q.read { db in
            ...
            XCTAssertTrue(try db.tableExists("pendingThreadOp"), "v39 must add pendingThreadOp")
            XCTAssertTrue(try db.tableExists("localDraft"), "v39 must add localDraft")
            ...
            XCTAssertTrue(try db.tableExists("threadSummary"), "v32 must add threadSummary")
            let summaryCols = try db.columns(in: "threadSummary").map(\.name)
            for name in ["threadId", "summary", "model", "updatedAt"] {
                XCTAssertTrue(summaryCols.contains(name), "v32 must add \(name)")
            }
        }
    }
